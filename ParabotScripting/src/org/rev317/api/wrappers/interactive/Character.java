...
public class Character implements Locatable, Interactable {
	...
	/**
	 * Returns the character which is interacted by this character
	 * @return id
	 */
	public final Character getInteractingCharacter() {
		int interactingId = accessor.getInteractingId();
		if (interactingId != -1 && interactingId < 32768) {
			return new Npc(Loader.getClient().getNpcs()[interactingId]);
		} else if (interactingId >= 32768) {
			interactingId -= 32768;
			if (interactingId == Loader.getClient().getLocalPlayerIndex()) {
				return Players.getLocal();
			}
			return new Player(Loader.getClient().getPlayers()[interactingId]);
		}
