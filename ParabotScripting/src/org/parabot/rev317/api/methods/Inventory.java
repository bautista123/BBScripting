...
public final class Inventory {
	...
	public static final int getCount(final boolean includeStack) {
		...
		final long[] stackSizes = includeStack ? inventory.getStackSizes() : null;
	...
	public static final Item[] getItems(final Filter<Item> filter) {
		...
		final int[] items = inventory.getItems();
		final long[] stackSizes = inventory.getStackSizes();
		...
		for(int i = 0; i < items.length; i++) {
			...
			final long stackSize = stackSizes[i];
			final Item item = new Item(itemId, (int)stackSize, i, Item.TYPE_INVENTORY);
